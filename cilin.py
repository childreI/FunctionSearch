...
class CilinSimilarity(object):
    ...
    def __init__(self):
        ...
        self.code_word = {}
        self.word_code = {}
        self.codes = [] #此行若是放在self.read_cilin()后面则会报错显示没有这个属性，所以要在前面
        self.vocab = set()
        self.N = 0
        self.read_cilin()#应该是一开始就得先执行这个函数
    ...
    def read_cilin(self):
        ...
        with open('cilin.txt', 'r', encoding='gbk') as f:
            for line in f.readlines():
                ...
                self.vocab.update(words)  #update()函数将特定str加入词典或集合
        ...
        """
         该函数从总编码库self.codes返回所有前五位相同的编码
        """
    def find_code(self, code):
        code_filter = filter(lambda x: x[0:5] == code, self.codes)
        return list(code_filter)
...
# sim2016 begin ===============================
    ...
    def dist2016(self, common_str):
        ...
        try :
            ...
            print('dist2016 error, 共有的层数不能大于5')

# sim2016 end ====================================

    def find_tongyici(self, w1):
        Code = self.get_code(w1)
        #print(Code)
        tongyici = []
        for i in list(map(lambda x: x[0:5], Code)):#分割编码的前八位，送到find_code函数中，得到所有前八位相同的编码
            #print(self.find_code(i))
            for j in self.find_code(i):#由编码得到对应的词汇列
                tongyici.extend(self.code_word[j])
        return tongyici

    def return_tongyici(self,word):
        tongyici = self.find_tongyici(word)
        print(tongyici)
        dic = {}
        test = How_Similarity()
        for i in tongyici:
            dic[i] = test.calc(word, '{}'.format(i))
        m = sorted(dic.items(),key=lambda item:item[1],reverse=True)
        n = []
        for i in m:
            if i[1] >= 0.5:#选取相似度大于0.5的同义词
                i = list(i)[0]
                n.append(i)
        return n

    def return_tongyici2(self, word):
        tongyici = self.find_tongyici(word)
        #print(tongyici)
        dic = {}
        test = How_Similarity()
...
if __name__ == '__main__':
    ...
    test = How_Similarity()
    for i in tongyici:
        dic[i] = test.calc('物体', '{}'.format(i))
    print(dic)
    print(sorted(dic.items(),key=lambda item:item[1],reverse=True))#根据hownet计算相似度排序
    '''
    ...
    #print(w1, '的编码有：', Code)
    tongyici = []
    for i in list(map(lambda x: x[0:5], Code)):
        for j in cs.find_code(i):
            tongyici.append(cs.code_word[j])
    basket = []
    for i in tongyici:
        basket.append(i[0])
    print(basket)
    for i in basket:
        print(cs.similarity(w1, i))
